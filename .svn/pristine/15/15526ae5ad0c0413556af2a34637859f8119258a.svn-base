...
public class ModelController extends BaseController{
    ...
    public ResultObject remove(@PathVariable("id") String id) {
        repositoryService.deleteModel(id);
        return ResultObject.ok();
    }

    @PostMapping("/model/deploy/{id}")
    public ResultObject deploy(@PathVariable("id") String id) throws Exception {
        //获取模型
        Model modelData = repositoryService.getModel(id);
        byte[] bytes = repositoryService.getModelEditorSource(modelData.getId());

        if (bytes == null) {
            return ResultObject.error("模型数据为空，请先设计流程并成功保存，再进行发布。");
        }
        ...
        if (model.getProcesses().size() == 0) {
            return ResultObject.error("数据模型不符要求，请至少设计一条主线流程。");
        }
