...
import cn.wewintech.activiti.service.ProcessService;
...
public class ProcessController extends BaseController{
    ...
    public ResultObject deploy(String exportDir, String category, MultipartFile file) {
        String message = "";
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
        ...
        return ResultObject.ok(message);
    }
    ...
    public ResultObject convertToModel(@PathVariable("procDefId") String procDefId, RedirectAttributes redirectAttributes) throws UnsupportedEncodingException, XMLStreamException {
        org.activiti.engine.repository.Model modelData = null;
        try {
            modelData = processService.convertToModel(procDefId);
            return ResultObject.ok( "转换模型成功，模型ID=" + modelData.getId());
        } catch (Exception e) {
            e.printStackTrace();
            return ResultObject.ok( "转换模型失败");
        }
    ...
    public ResultObject batchRemove(@RequestParam("ids[]") String[] ids) {
        for (String id : ids) {
            repositoryService.deleteDeployment(id,true);
        }
        return ResultObject.ok();
